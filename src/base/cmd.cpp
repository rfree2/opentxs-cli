...
cCmdParser::~cCmdParser() {
} // let's instantize default destr in all TUs so compilation will fail (without this line) on unique_ptr on not-complete types trololo - B. Stroustrup
...
cCmdProcessing cCmdParser::StartProcessing(const string &words, shared_ptr<
		nUse::cUseOT> use) {
...
cCmdName::cCmdName(const string &name) :
		mName(name) {
}

bool cCmdName::operator<(const cCmdName &other) const {
	return mName < other.mName;
}

cCmdName::operator std::string() const {
	return mName;
}

// ========================================================================================================================

cCmdProcessing::cCmdProcessing(shared_ptr<cCmdParser> parser, const string &commandLineString, shared_ptr<
		nUse::cUseOT> use) :
		mStateParse(tState::never), mStateValidate(tState::never), mStateExecute(tState::never), mFailedAfterBadCmdname(false), mParser(parser), mCommandLineString(commandLineString), mUse(use) {
...
void cCmdProcessing::Validate() {
	if (mStateValidate != tState::never) {
		_dbg1("Validation was done already");
		return;
	}
...
void cCmdProcessing::_Validate() {
	if (mStateParse != tState::succeeded)
		Parse();
	if (mStateParse != tState::succeeded) {
		_dbg1("Failed to parse.");
	}

	if (!mData) {
		_warn("Can not validate - there is no mData");
		return;
	}
	if (!mFormat) {
		_warn("Can not validate - there is no mData");
...
void cCmdProcessing::_Parse(bool allowBadCmdname) {
	...
	{ // initial parsing
		...
		for (size_t pos = 0; pos < mCommandLineString.size(); ++pos) { // each character
			...
			if (c == ' ') { // white char
				...
				if (curr_word.size() == 0)
					curr_word_pos = pos; // this is the first character of current (new) word
				curr_word += c;
			}
		} // for each char
	...
	// -----------------------------------
	if (mCommandLine.at(0) == "help") {
		mData->mCharShift = -3;
		namepart_words--;
		prepart_words--;
		mCommandLine.insert(mCommandLine.begin(), "ot");
	} // change "help" to "ot help"
	...
	try {
		...
		try {
			...
			if (allowBadCmdname) {
				mFailedAfterBadCmdname = true;
				return; // <=== RETURN.  exit, but report that we given up early  <===========================
			} else {
				throw;
		...
		if (phase == 3) {
			string prev_name = "";
			bool inside_opt = false; // are we now in middle of --option ?  curr_name is the opt name like "--cc"
			int token_nr = 0;
...
vector<string> cCmdProcessing::UseComplete(int char_pos) {
	...
	try {
		...
		if (allwords.at(0) == "help")
			return {}; // there is nothing to complete after "help" alias
		...
		const string word_previous =
				(word_previous_ixtab >= 0) ?
						mCommandLine.at(word_previous_ixtab) : "";
		...
		if (entity.mKind == cParseEntity::tKind::fake_empty) { // we are finishg an empty word - we are now creating a new word
			...
			} else if (entity_previous.mKind == cParseEntity::tKind::cmdname) { // after command name: 2nd word of command name or jump further to e.g. var/varExt/opt
		...
		if (entity.mKind == cParseEntity::tKind::argument_somekind) {
			...
			if (!mFormat) { // we do not yet have a format even
				...
				if (allwords.size() <= 2)
					entity.SetKind(cParseEntity::tKind::cmdname); // in commad name... is 1 correct?
			} else {
				ASRT(mFormat);
				if (arg_nr <= xsize_t(mFormat->mVar.size())) {
					entity.SetKind(cParseEntity::tKind::variable);
				} else if (arg_nr <= xsize_t(mFormat->SizeAllVar())) {
					entity.SetKind(cParseEntity::tKind::variable_ext);
				} else {
					entity.SetKind(cParseEntity::tKind::option_name);
				}
			}
		}
		...
		if (entity.mKind == cParseEntity::tKind::option_name) {
			...
			if (!format)
				return vector<string> { }; // if we did not understood command name, then return empty vector
			...
			} // something failed probaly the option name was not known
			...
			_info("Completing variable as arg_nr="<<arg_nr);
			ASRT(mFormat);
			...
			if ((cmd_word_nr == 0)) { // "~" like from "ot ~"
				...
				throw cErrInternalParse("Bad cmd_word_nr=" + ToStr(cmd_word_nr) + ", after_word=" + ToStr(after_word) + " in completion");
	...
	_erro("Dead code hit?");
	return vector<string>(); // should not happen
}

void cCmdProcessing::UseExecute() { // TODO write as a template for all the 3 wrappres that set state ??
	if (mStateParse != tState::succeeded)
		Parse();
	if (mStateValidate != tState::succeeded)
		Validate();
	if (mStateParse != tState::succeeded) {
		_dbg1("Failed to parse.");
	}
	if (mStateValidate != tState::succeeded) {
		_dbg1("Failed to validate.");
	}

	if (mStateExecute != tState::never) {
		_dbg1("Exec was done already");
		return;
	}
...
void cCmdProcessing::_UseExecute() {
	if (!mFormat) {
		_warn("Can not execute this command - mFormat is empty");
		return;
	}
...
cParamInfo cParamInfo::operator<<(const cParamInfo &B) const {
	...
	if (B.funcValid)
		A.funcValid = B.funcValid;
	if (B.funcHint)
		A.funcHint = B.funcHint;
	return A;
}

bool cParamInfo::IsValid() const {
	if (mName.length() < 1) {
		_warn("Invalid cParamInfo with empty name!");
		return false;
	}
	return true;
}
...
bool cCmdFormat::IsValid() const {
	bool allok = true;
	for (const auto & elem : mVar)
		if (!elem.IsValid())
			allok = false;
	for (const auto & elem : mVarExt)
		if (!elem.IsValid())
			allok = false;
	for (const auto & elem : mOption)
...
cParamInfo cCmdFormat::GetParamInfo(int nr) const {
	...
	if (nr <= 0)
		throw myexception("Illegal number for var, nr=" + ToStr(nr));
	const int ix = nr - 1;
	//xsize_t xst; xst.m_v(ix);
...
void cCmdFormat::PrintUsageLong(ostream &out) const {
	...
	for (int sort = 0; sort <= 1; ++sort) {
		...
		for (auto opt : mOption) {
			...
			if ((int) sort != boring)
				continue; // first 0 then 1

			out << color1 << std::setw(width1) << name; // --cc
			if (info.getTakesValue())
				out << " " << cc::fore::lightcyan << info.getName() << color1; // username
...
cCmdExecutable::cCmdExecutable(tFunc func) :
		mFunc(func) {
}
...
void cCmdData::AssertLegalOptName(const string &name) const {
	if (name.size() < 1)
		throw cErrArgIllegal("option name can not be empty");
	const size_t maxlen = 100;
	if (name.size() > maxlen)
		throw cErrArgIllegal("option name too long, over" + ToStr(maxlen));
...
string cCmdData::Opt1If(const string& name, const string &def) const { // same but requires the 1st element; therefore we need def argument again
	...
	if (vec.size() < 1) {
		_warn("Not normalized opt for name="<<name);
		return def;
	}
	return vec.at(0);
}
...
vector<string> cCmdData::Opt(const string& name) const {
	...
	if (find == mOption.end()) {
		_warn("Map was: [TODO]");
		throw cErrArgMissing("Option " + name + " was missing");
	}
	return find->second;
}
...
string cCmdData::Opt1(const string& name) const {
	...
	if (vec.size() < 1) {
		_warn("Not normalized opt for name="<<name);
		throw cErrArgMissing("Option " + name + " was missing (not-normalized empty vector)");
	}
	return vec.at(0);
}
...
int cCmdDataParse::WordIx2ArgNr(int word_ix) const {
	if (word_ix < 0)
		throw cErrAfterparse("Trying to use word_ix=" + ToStr(word_ix));
	int arg_nr = word_ix - mFirstArgAfterWord;
	_dbg3("mFirstArgAfterWord="<<mFirstArgAfterWord);
	if (arg_nr < 0)
		arg_nr = 0; // arg number 0 will mean "part of the name"
	// ASRT(arg_nr >=0 );
	return arg_nr;
}
